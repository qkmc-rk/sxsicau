...
CREATE TABLE `sx_dict_campus` (
  ...
  `sx_dict_campus` varchar(8) NOT NULL COMMENT '校区',
  PRIMARY KEY (`sx_dict_id`)
...
CREATE TABLE `sx_dict_college` (
  ...
  `sx_dict_college` varchar(18) NOT NULL COMMENT '学院',
  PRIMARY KEY (`sx_dict_id`,`sx_dict_college`),
  KEY `sx_dict_id` (`sx_dict_id`),
  KEY `fk_campus` (`sx_dict_campus_id`),
...
CREATE TABLE `sx_dict_grade` (
  `sx_dict_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `sx_dict_grade` varchar(8) NOT NULL COMMENT '年级',
...
CREATE TABLE `sx_dict_major` (
  ...
  `sx_dict_major` varchar(32) NOT NULL COMMENT '专业',
  PRIMARY KEY (`sx_dict_id`),
  KEY `fk_college` (`sx_dict_college_id`),
...
CREATE TABLE `sx_dict_nation` (
  `sx_dict_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `sx_dict_nation` varchar(8) NOT NULL COMMENT '名族名称',
  PRIMARY KEY (`sx_dict_id`)
...
CREATE TABLE `sx_dict_sex` (
  `sx_dict_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `sx_dict_sex` varchar(8) NOT NULL COMMENT '性别',
  PRIMARY KEY (`sx_dict_id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8;
...
CREATE TABLE `sx_ext_comment` (
  ...
  `sx_ext_commenter_id` varchar(32) NOT NULL COMMENT '评论者的学号',
  `sx_ext_comment` text COMMENT '评论内容',
  `sx_ext_gmt_create` datetime DEFAULT NULL COMMENT '评论时间',
...
CREATE TABLE `sx_ext_follow` (
  ...
  `sx_ext_student_id` varchar(32) NOT NULL COMMENT '学生学号',
  `sx_ext_followee_id` varchar(32) NOT NULL COMMENT '关注的人的id',
  `sx_ext_gmt_modified` datetime DEFAULT NULL,
  `sx_ext_gmt_create` datetime DEFAULT NULL COMMENT '关注时间',
  `sx_ext_is_following` tinyint(2) unsigned NOT NULL DEFAULT '1' COMMENT '关注状态 1在关注  0取消关注',
  PRIMARY KEY (`sx_ext_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='关注表，学生可以关注别的学生，关注了就可以看到他的动态';
...
CREATE TABLE `sx_ext_recommend` (
  `sx_ext_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `sx_ext_rcmd_stu_id` varchar(32) NOT NULL COMMENT '被推荐学生得ID',
  `sx_ext_gmt_modified` datetime DEFAULT NULL COMMENT '修改时间',
  `sx_ext_gmt_create` datetime DEFAULT NULL COMMENT '推荐时间',
  `sx_ext_root_id` int(11) unsigned NOT NULL COMMENT '操作者ID(管理员)',
  `sx_ext_rcmder_id` varchar(255) NOT NULL COMMENT '推荐老师的ID',
...
CREATE TABLE `sx_ext_reply` (
  ...
  `sx_ext_replyer_id` varchar(32) NOT NULL COMMENT '回复者学号',
  `sx_ext_commender_id` varchar(32) NOT NULL COMMENT '评论者的ID',
  `sx_ext_msg` text COMMENT '回复信息',
  `sx_ext_gtm_create` datetime DEFAULT NULL COMMENT '回复时间',
...
CREATE TABLE `sx_practice` (
  ...
  `sx_student_id` varchar(32) NOT NULL,
  `sx_student_name` varchar(12) DEFAULT NULL COMMENT '学生姓名',
  `sx_major` varchar(32) DEFAULT NULL COMMENT '学生专业',
  `sx_title` varchar(32) DEFAULT NULL COMMENT '实习报告标题',
  `sx_report` text COMMENT '报告正文',
  `sx_picture` varchar(48) DEFAULT NULL COMMENT '实习照片',
  `sx_video` varchar(48) DEFAULT NULL COMMENT '实习视频',
  `sx_gtm_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `sx_gtm_create` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '实习创建时间',
  ...
  PRIMARY KEY (`sx_id`),
  KEY `fk_to_student` (`sx_student_id`),
...
CREATE TABLE `sx_practice_score` (
  `sx_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
  `sx_teacher_id` varchar(32) DEFAULT NULL COMMENT '指导老师教师号',
  `sx_practice_id` int(11) unsigned NOT NULL COMMENT '关联每一个practice得sx_id',
  `sx_gtm_create` datetime DEFAULT NULL COMMENT '评分时间',
  `sx_score` decimal(10,0) unsigned NOT NULL COMMENT '实习分数',
  `sx_remark` text COMMENT '实习点评',
  `sx_gtm_modified` datetime DEFAULT NULL,
  PRIMARY KEY (`sx_id`),
  KEY `fk_to_practice` (`sx_practice_id`),
  CONSTRAINT `fk_to_practice` FOREIGN KEY (`sx_practice_id`) REFERENCES `sx_practice` (`sx_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='每一个小的practice打分得表';
...
CREATE TABLE `sx_root` (
  ...
  `sx_account` varchar(32) NOT NULL COMMENT '管理员账号',
  `sx_password` varchar(48) NOT NULL COMMENT '管理员密码',
  `sx_is_lock` tinyint(2) unsigned NOT NULL DEFAULT '0' COMMENT '1-锁定不可用',
  `sx_gtm_create` datetime DEFAULT NULL,
  `sx_gtm_modified` datetime DEFAULT NULL,
  PRIMARY KEY (`sx_id`)
...
CREATE TABLE `sx_student` (
  `sx_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
  `sx_student_id` varchar(32) NOT NULL COMMENT '学号',
  `sx_student_name` varchar(12) NOT NULL COMMENT '姓名',
  `sx_student_phone` varchar(16) NOT NULL COMMENT '学生联系电话',
  `sx_student_password` varchar(48) NOT NULL COMMENT '学生登录密码',
  `sx_sex` varchar(8) NOT NULL COMMENT '性别',
  `sx_nation` varchar(8) NOT NULL,
  `sx_grade` varchar(16) NOT NULL,
  `sx_college` varchar(32) NOT NULL,
  `sx_major` varchar(32) NOT NULL,
  `sx_clazz` varchar(16) NOT NULL,
  `sx_campus` varchar(12) NOT NULL,
  `sx_picture` varchar(48) DEFAULT NULL,
  `sx_is_lock` tinyint(2) unsigned NOT NULL DEFAULT '0' COMMENT '1-未启用',
  `sx_gtm_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `sx_gtm_modified` datetime DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  `sx_totor_up` tinyint(3) unsigned NOT NULL DEFAULT '5' COMMENT '学生最多能选择多少导师',
  PRIMARY KEY (`sx_id`),
  UNIQUE KEY `sx_student_id` (`sx_student_id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=56 DEFAULT CHARSET=utf8 COMMENT='学生信息表';
...
CREATE TABLE `sx_teacher` (
  `sx_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
  `sx_teacher_id` varchar(32) NOT NULL COMMENT '教师员工号',
  `sx_teacher_name` varchar(12) NOT NULL COMMENT '教师姓名',
  `sx_teacher_password` varchar(48) NOT NULL COMMENT '教师登录密码',
  `sx_college` varchar(16) NOT NULL COMMENT '教师所属学院',
  `sx_nation` varchar(8) NOT NULL COMMENT '教师民族',
  `sx_sex` varchar(8) NOT NULL COMMENT '教师性别',
  `sx_age` varchar(8) NOT NULL COMMENT '教师年龄',
  `sx_gtm_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `sx_gtm_modified` datetime DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  ...
  PRIMARY KEY (`sx_id`),
  UNIQUE KEY `sx_teacher_id` (`sx_teacher_id`) USING BTREE
...
CREATE TABLE `sx_teacher_resume` (
  ...
  `sx_teacher_id` varchar(32) NOT NULL COMMENT '关联教师',
  `sx_picture` varchar(255) DEFAULT NULL COMMENT '简历照片地址',
  `sx_gtm_modified` datetime NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  `sx_gtm_create` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '简历更新时间',
  `sx_name` varchar(12) NOT NULL COMMENT '姓名',
  `sx_sex` varchar(8) DEFAULT NULL COMMENT '性别',
  `sx_birth` varchar(32) DEFAULT NULL COMMENT '出生日期',
  `sx_type` varchar(32) DEFAULT NULL COMMENT '导师类型，如学硕导师，博士导师，本科生导师等等',
  `sx_unit` varchar(32) DEFAULT NULL COMMENT '工作单位',
  `sx_admin` varchar(120) DEFAULT NULL COMMENT '行政职务，如农学院党委副书记兼副院长',
  `sx_admin_phone` varchar(18) DEFAULT NULL COMMENT '单位电话',
  `sx_tech_major` varchar(255) DEFAULT NULL COMMENT '招生专业',
  `sx_email` varchar(32) DEFAULT NULL COMMENT '电子邮件',
  `sx_introduce` text COMMENT '个人简介',
  `sx_work_exp` text COMMENT '工作经历',
  `sx_edu_exp` text COMMENT '教育经历',
  `sx_award` text COMMENT '获奖荣誉',
  `sx_other_status` text COMMENT '社会、学会及学术兼职',
  `sx_research_area` text COMMENT '研究领域',
  `sx_sci_resch_project` text COMMENT '科研项目',
  `sx_paper` text COMMENT '发表论文',
  `sx_book` text COMMENT '专著教材',
  `sx_students` text COMMENT '指导学生',
  `sx_is_visible` tinyint(2) unsigned NOT NULL DEFAULT '1' COMMENT '简历是否可见 1-可见',
  PRIMARY KEY (`sx_id`),
  KEY `fk_teacher` (`sx_teacher_id`),
...
CREATE TABLE `sx_total_score` (
  `sx_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `sx_teacher_id` varchar(32) DEFAULT NULL COMMENT '评分教师教职工编号',
  `sx_student_id` varchar(32) NOT NULL COMMENT '学生学号',
  `sx_total_score` decimal(10,0) unsigned NOT NULL COMMENT '实习总分',
  `sx_total_remark` text COMMENT '实习总评',
  `sx_gtm_create` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '总评时间',
  `sx_gtm_modified` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '总评修改时间',
  PRIMARY KEY (`sx_id`),
  KEY `fk_toStudent` (`sx_student_id`),
  KEY `fk_toTeacher` (`sx_teacher_id`),
  CONSTRAINT `fk_toStudent` FOREIGN KEY (`sx_student_id`) REFERENCES `sx_student` (`sx_student_id`),
...
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8 COMMENT='学生实习总评分表';
...
CREATE TABLE `sx_tutor` (
  `sx_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `sx_student_id` varchar(32) NOT NULL COMMENT '学生id',
  `sx_teacher_id` varchar(32) NOT NULL COMMENT '老师教职工号',
  ...
  `sx_gtm_create` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `sx_gtm_modified` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `wx_user` (
  ...
  `binding_number` varchar(255) DEFAULT NULL COMMENT '绑定的学号或者教师号码',
